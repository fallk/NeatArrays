...
import fallk.neatarrays.$primitiveFmt$Iterator;
import fallk.neatarrays.$primitiveFmt$Spliterator;
 ...
 *
 * <p>Most {@code $primitiveFmt$Deque} operations run in amortized constant time.
 * Exceptions include {@link #remove($primitive$) remove}, {@link
 ...
 * @since   1.6
 * @param <$primitive$> the type of elements held in this collection
 */
public class $primitiveFmt$Deque implements Cloneable, Serializable
{
    // the primitive value that represents 'null'
    protected static final $primitive$ NULL_VALUE = $primitiveWrp$.MIN_VALUE;
     ...
     * of the deque (via addLast($primitive$), add($primitive$), or push($primitive$)).
     */
    transient int tail;
     ...
     * @return its argument
     */
    private $primitive$[] copyElements($primitive$[] a) {
        if (head < tail) {
     ...
     */
    public $primitiveFmt$Deque(int numElements) {
     ...
     */
    public $primitiveFmt$Deque(Collection<$primitiveWrp$> c) {
        allocateElements(c.size());
        addAll(c);
    }

    public void addAll(Collection<$primitiveWrp$> c) {
        for ($primitiveWrp$ s : c) {
            add(s);
        }
    }
     ...
     * @throws NullPointerException if the specified element is NULL_VALUE
     */
    public boolean offerFirst($primitive$ e) {
        addFirst(e);
        return true;
    }
     ...
     * @throws NullPointerException if the specified element is NULL_VALUE
     */
    public boolean offerLast($primitive$ e) {
        addLast(e);
        return true;
    }
     ...
     * @throws NullPointerException if the specified element is NULL_VALUE
     */
    public boolean offer($primitive$ e) {
        return offerLast(e);
    }
     ...
     */
    public $primitiveFmt$Iterator iterator() {
        return new DeqIterator();
    }

    public $primitiveFmt$Iterator descendingIterator() {
        return new DescendingIterator();
    }

    private class DeqIterator implements $primitiveFmt$Iterator {
        /**
    ...
    private class DescendingIterator implements $primitiveFmt$Iterator {
        /*
     ...
     *
     * @param o $primitive$ to be checked for containment in this deque
     ...
     *
     * <p>This method is equivalent to {@link #removeFirstOccurrence($primitive$)}.
     *
    ...
    public $primitive$[] toArray() {
        return copyElements(new $primitive$[size()]);
    }
     ...
     */
    public $primitiveFmt$Deque clone() {
        try {
            
            $primitiveFmt$Deque result = ($primitiveFmt$Deque) super.clone();
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        for (int i = 0; i < size; i++)
            elements[i] = s.read$primitiveFmt$();
    }
     ...
     * @since 1.8
     */
    public $primitiveFmt$Spliterator spliterator() {
        return new DeqSpliterator(this, -1, -1);
    }

    static final class DeqSpliterator implements $primitiveFmt$Spliterator {
        private final $primitiveFmt$Deque deq;
        private int fence;  // -1 until first use
        private int index;  // current index, modified on traverse/split

        /** Creates new spliterator covering the given array and range */
        DeqSpliterator($primitiveFmt$Deque deq, int origin, int fence) {
            this.deq = deq;
            this.index = origin;
            this.fence = fence;
        }
        ...
        public void forEachRemaining(Consumer<$primitiveWrp$> consumer) {
            if (consumer == null)
                throw new NullPointerException();
            $primitive$[] a = deq.elements;
        ...
        public boolean tryAdvance(Consumer<$primitiveWrp$> consumer) {
            if (consumer == null)
                throw new NullPointerException();
            $primitive$[] a = deq.elements;
            int m = a.length - 1;
