 ...
 *
 * <p>Most {@code IntDeque} operations run in amortized constant time.
 * Exceptions include {@link #remove(int) remove}, {@link
 ...
 * @since   1.6
 * @param <int> the type of elements held in this collection
 */
public class IntDeque implements Cloneable, Serializable
{
     ...
     * of the deque (via addLast(int), add(int), or push(int)).
     */
    transient int tail;
     ...
     */
    public static void intFill(int[] array) {
     ...
     */
    public IntDeque(Collection<Integer> c) {
    ...
    public int getLast() {
        ...
        int result = (int) elements[(tail - 1) & (elements.length - 1)];
        if (result == NULL_VALUE)
            throw new NoSuchElementException();
        return result;
    }
    ...
    public int peekLast() {
        return (int) elements[(tail - 1) & (elements.length - 1)];
    }
    ...
    private class DeqIterator implements IntIterator {
        ...
        public int next() {
            ...
            int result = (int) elements[cursor];
     ...
     *
     * @param o int to be checked for containment in this deque
     ...
     */
    public IntDeque clone() {
        try {
            
            IntDeque result = (IntDeque) super.clone();
    ...
    static final class DeqSpliterator implements IntSpliterator {
        private final IntDeque deq;
        private int fence;  // -1 until first use
        private int index;  // current index, modified on traverse/split

        /** Creates new spliterator covering the given array and range */
        DeqSpliterator(IntDeque deq, int origin, int fence) {
            this.deq = deq;
            this.index = origin;
            this.fence = fence;
        }
        ...
        public void forEachRemaining(Consumer<Integer> consumer) {
            if (consumer == null)
                throw new NullPointerException();
            int[] a = deq.elements;
        ...
        public boolean tryAdvance(Consumer<Integer> consumer) {
            if (consumer == null)
                throw new NullPointerException();
            int[] a = deq.elements;
            int m = a.length - 1;
